...
public enum ZoneKitSourceType: Sendable, Hashable {
...
///
/// Provided by ZoneKit via delegate; use to request start/stop capture.
/// The `type` property uses `CaptureSourceType` enum for easy switching.
public struct ZoneKitSource: Sendable, Hashable, Identifiable {
    ...
    public let type: ZoneKitSourceType
...
public enum ZoneKitSourceState: Sendable {
    ...
    /// ZoneKit will attempt automatic reconnection
    case interrupted

    /// Source encountered an error
    case error(ZoneKitError)
}

// MARK: - ZoneKitError

/// Errors that can occur during capture operations
public enum ZoneKitError: Error, Sendable {
...
/// Permission types that ZoneKit may require
...
public enum ZoneKitDeviceFamily: String, CaseIterable {
    // iPhones
...
public enum ZoneKitOrientation {
    case portrait, landscape
}

enum ZoneKitPixelFormat {
    case rgb
    case yCbCr
...
struct ZoneKitVideoSpec {
    var fps: Int32?
    var size: CGSize?
}

struct ZoneKitCapturedFrame: @unchecked Sendable {
    static var invalid: ZoneKitCapturedFrame {
        ZoneKitCapturedFrame(surface: nil, contentRect: .zero, contentScale: 0, scaleFactor: 0)
    }
