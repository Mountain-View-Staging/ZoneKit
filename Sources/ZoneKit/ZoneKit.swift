...
public protocol ZoneKitDelegate: AnyObject {
    func zoneKitDidInitialize(_ manager: ZoneKit)
    func zoneKitScreenDidOutputSampleBuffer(sampleBuffer: CMSampleBuffer, source: ZoneKitSource)
    func captureSourceListDidChange(_ manager:ZoneKit, sources: [ZoneKitSource])
    func zoneKitDeviceDidOutputSampleBuffer(sampleBuffer: CMSampleBuffer, source: ZoneKitSource)
    func zoneKit(_ manager: ZoneKit, permissionStatusDidChange type: PermissionType, status: PermissionStatus)
    #if os(macOS)
...
public extension ZoneKitDelegate {
    func zoneKitDeviceDidOutputSampleBuffer(sampleBuffer: CMSampleBuffer, source: ZoneKitSource) {}
    func zoneKit(_ manager: ZoneKit, permissionStatusDidChange type: PermissionType, status: PermissionStatus) {}
    #if os(macOS)
    func zoneKitVirtualDisplayDidOutputTexture(texture: MTLTexture, source: ZoneKitSource) {}
...
public final class ZoneKit: AVCaptureSession {
    
    // MARK: - Properties
    
    public weak var delegate: ZoneKitDelegate?
    ...
    #if os(macOS)
    private var captureScreens: [ZoneKitScreen] = []
    private var captureVirtualDisplays: [ZoneKitVirtualDisplay] = []
    #endif
    
    public var captureSources: [ZoneKitSource] = []
    private var captureDevices: [ZoneKitDevice] = []
    ...
    public convenience init(delegate: ZoneKitDelegate) {
        self.init()
        ...
        if #unavailable(iOS 17.0) {
            ...
            delegate.zoneKitDidInitialize(self)
            return
        }
        #endif
    ...
    public func enableCapture(for source: ZoneKitSource) {
        ...
        case .virtualDisplay:
            #if os(macOS)
            guard let virtualDisplay = captureVirtualDisplays.first(where: {
                $0.captureSource?.id == source.id
            }) else {
                mlog.error("Could not find virtual display for source: \(source.id)")
                enabledSources.remove(source.id)
                return
            }
            virtualDisplay.startCapture()
            mlog.info("Started virtual display capture for: \(source.displayName)")
            #else
            mlog.debug("Virtual display capture not available on this platform")
            enabledSources.remove(source.id)
            #endif
        }
    }

    public func disableCapture(for source: ZoneKitSource) {
        ...
        case .virtualDisplay:
            ...
            virtualDisplay.stopCapture()
            mlog.info("Stopped virtual display capture for: \(source.displayName)")
            #else
            mlog.debug("Virtual display capture not available on this platform")
            #endif
        }
    ...
    public func disableCapture(for source: ZoneKitSource, completion: @escaping () -> Void) {
        mlog.info("Disabling capture for source: \(source.displayName) (type: \(String(describing: source.type)))")
        enabledSources.remove(source.id)
    ...
    private func addToSession(captureDevice: ZoneKitDevice) {
    ...
    private func removeFromSession(captureDevice: ZoneKitDevice, completion: @escaping () -> Void) {
        sessionQueue.async { [weak self] in
            guard let self = self else { return }
    ...
    private func screenFound(displayId: CGDirectDisplayID) {
        ...
        let newCapture = ZoneKitScreen(delegate: self, displayId: displayId)
    ...
    func emitCaptureSources() {
        var emittableList = [ZoneKitSource]()
        var seenIDs = Set<String>()

        // Debug: Log current state of arrays
        #if os(macOS)
        mlog.debug("Emitting sources — screens: \(self.captureScreens.count), virtualDisplays: \(self.captureVirtualDisplays.count), devices: \(self.captureDevices.count)")
        ...
        for (index, vd) in captureVirtualDisplays.enumerated() {
            if let source = vd.captureSource {
                mlog.debug("  VirtualDisplay[\(index)]: \(source.displayName)")
            }
        }
        #else
        ...
        // Add virtual display sources
        for virtualDisplay in captureVirtualDisplays {
            if let captureSource = virtualDisplay.captureSource,
               !seenIDs.contains(captureSource.id) {
                emittableList.append(captureSource)
                seenIDs.insert(captureSource.id)
            } else if let captureSource = virtualDisplay.captureSource {
                mlog.debug("Skipping duplicate virtual display source: \(captureSource.displayName)")
            }
        }
        #endif
    ...
    func deviceFound(device: AVCaptureDevice) {
        ...
        let captureDevice = ZoneKitDevice(device: device, delegate: self)
    ...
    // MARK: - Virtual Display Management (macOS)

    #if os(macOS)
    /// Creates a new virtual display source and registers it with ZoneKit.
    ///
    /// The virtual display is created but capture does not start until
    /// `enableCapture(for:)` is called with the returned source.
    ///
    /// - Parameters:
    ///   - identifier: Unique string identifier for this virtual display
    ///   - configuration: Display configuration (defaults to 1080p)
    /// - Returns: The `ZoneKitSource` descriptor for the new virtual display, or `nil` if creation failed
    @discardableResult
    public func addVirtualDisplay(
        identifier: String,
        configuration: ZoneKitVirtualDisplayConfiguration = .preset1080p
    ) -> ZoneKitSource? {
        // Prevent duplicates
        guard !captureVirtualDisplays.contains(where: {
            $0.captureSource?.id == "virtual-display-\(identifier)"
        }) else {
            mlog.warning("Virtual display already exists with identifier: \(identifier)")
            return captureVirtualDisplays.first(where: {
                $0.captureSource?.id == "virtual-display-\(identifier)"
            })?.captureSource
        }

        let virtualDisplay = ZoneKitVirtualDisplay(
            identifier: identifier,
            delegate: self,
            configuration: configuration
        )

        guard let source = virtualDisplay.captureSource else {
            mlog.error("Failed to create capture source for virtual display: \(identifier)")
            return nil
        }

        captureVirtualDisplays.append(virtualDisplay)

        // Start capture immediately — the virtual display needs to be active
        // so it appears in NSScreen.screens and CaptureKit can discover it.
        virtualDisplay.startCapture()

        mlog.info("Added virtual display: \(identifier). Total virtual displays: \(captureVirtualDisplays.count)")

        emitCaptureSources()
        return source
    }

    /// Removes a virtual display by identifier.
    ///
    /// Stops capture if active and removes the virtual display from ZoneKit.
    /// - Parameter identifier: The identifier used when the display was created
    public func removeVirtualDisplay(identifier: String) {
        let sourceID = "virtual-display-\(identifier)"

        guard let index = captureVirtualDisplays.firstIndex(where: {
            $0.captureSource?.id == sourceID
        }) else {
            mlog.warning("No virtual display found with identifier: \(identifier)")
            return
        }

        let virtualDisplay = captureVirtualDisplays[index]

        // Stop capture if active
        virtualDisplay.stopCapture { [weak self] in
            guard let self = self else { return }

            // Clean up enabledSources
            self.enabledSources.remove(sourceID)

            // Remove from array
            self.captureVirtualDisplays.removeAll { $0.captureSource?.id == sourceID }

            mlog.info("Removed virtual display: \(identifier). Total virtual displays: \(self.captureVirtualDisplays.count)")

            self.emitCaptureSources()
        }
    }

    /// Creates a preview window for a virtual display.
    ///
    /// The window contains a `ZoneKitVirtualDisplayPreviewView` that renders IOSurface
    /// frames from the virtual display's CGDisplayStream. Click coordinates in the preview
    /// are available via the view's `onTap` callback for cursor relay.
    ///
    /// - Parameters:
    ///   - identifier: The virtual display identifier used in `addVirtualDisplay`.
    ///   - title: Window title.
    ///   - initialSize: Initial window size in points.
    ///   - minSize: Minimum window size in points.
    /// - Returns: A configured NSWindow, or nil if the virtual display was not found.
    public func createPreviewWindow(
        for identifier: String,
        title: String = "Virtual Display",
        initialSize: CGSize = CGSize(width: 960, height: 540),
        minSize: CGSize = CGSize(width: 480, height: 270)
    ) -> NSWindow? {
        let sourceID = "virtual-display-\(identifier)"
        guard let virtualDisplay = captureVirtualDisplays.first(where: {
            $0.captureSource?.id == sourceID
        }) else {
            mlog.warning("createPreviewWindow — no virtual display found: \(identifier)")
            return nil
        }

        // Determine resolution from configuration
        let resolution = CGSize(
            width: CGFloat(virtualDisplay.configuration.maxWidth),
            height: CGFloat(virtualDisplay.configuration.maxHeight)
        )

        // Create preview view
        let previewView = ZoneKitVirtualDisplayPreviewView()
        previewView.attach(virtualDisplay, resolution: resolution)

        // Note: Click-to-cursor relay is intentionally NOT wired here.
        // CGDisplayMoveCursorToPoint moves the system cursor to the virtual display,
        // causing macOS to activate whatever app owns that display's frontmost surface
        // (typically Finder/Desktop), stealing focus from the preview window.
        // Consumers should wire onTap/onDoubleTap externally with their own focus
        // management (e.g., NSApp.activate after cursor move) if cursor relay is needed.
        ...
        mlog.info("createPreviewWindow — \(identifier), \(Int(resolution.width))×\(Int(resolution.height))")
        return window
    }
    #endif
...
#if os(macOS)
extension ZoneKit: ZoneKitScreenDelegate {
    func zoneKitScreenDidOutputSampleBuffer(sampleBuffer: CMSampleBuffer, source: ZoneKitSource) {
        guard let delegate else {
            mlog.error("CaptureKitScreenDelegate: no delegate found")
            return
        }
        delegate.zoneKitScreenDidOutputSampleBuffer(sampleBuffer: sampleBuffer, source: source)
    }
}
#endif

extension ZoneKit: ZoneKitDeviceDelegate {
        ...
        if let captureDevice = captureDevices.first(where: { $0.device.uniqueID == uniqueID }),
           let source = captureDevice.captureSource {
            ...
            if source.type == .iOSDevice {
                ...
                delegate.zoneKitDeviceDidOutputSampleBuffer(sampleBuffer: sampleBuffer, source: source)
                return
            }
...
#if os(macOS)
extension ZoneKit: ZoneKitVirtualDisplayDelegate {
    func zoneKitVirtualDisplayDidOutputTexture(texture: MTLTexture, source: ZoneKitSource) {
        guard let delegate else {
            mlog.error("ZoneKitVirtualDisplayDelegate: no delegate found")
            return
        }
    ...
    func zoneKitVirtualDisplayDidBecomeReady(source: ZoneKitSource, displayID: CGDirectDisplayID) {
        mlog.info("Virtual display ready: \(source.displayName) (displayID: \(displayID))")
...
extension ZoneKit: PermissionManagerDelegate {
    func permissionManager(_ manager: PermissionManager, didResolvePermission type: PermissionType, status: PermissionStatus) {
        ...
        delegate?.zoneKit(self, permissionStatusDidChange: type, status: status)
        ...
        case .camera:
            if status == .authorized {
                ...
                sessionQueue.async { [weak self] in
                    ...
                    DispatchQueue.main.async {
                        self.delegate?.zoneKitDidInitialize(self)
