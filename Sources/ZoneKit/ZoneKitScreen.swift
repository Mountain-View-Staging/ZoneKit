...
protocol ZoneKitScreenDelegate: AnyObject {
    nonisolated func zoneKitScreenDidOutputSampleBuffer(sampleBuffer: CMSampleBuffer, source: ZoneKitSource)
}

// MARK: - CaptureScreen

public class ZoneKitScreen: NSObject {
    
    // MARK: - CaptureSource / displayID
    
    nonisolated(unsafe) public private(set) var captureSource: ZoneKitSource?
    ...
    nonisolated(unsafe) weak var delegate: ZoneKitScreenDelegate?
    
    private let videoQueue = DispatchQueue(
        label: "com.capturekit.ZoneKitScreen.video",
        qos: .userInteractive
    )
    ...
    convenience init(delegate: ZoneKitScreenDelegate, displayId: CGDirectDisplayID? = nil) {
        self.init()
    ...
    private func createCaptureSource(for displayId: CGDirectDisplayID) -> ZoneKitSource? {
...
extension ZoneKitScreen: SCStreamDelegate {
...
@available(macOS 14.0, *)
extension ZoneKitScreen: SCStreamOutput {
